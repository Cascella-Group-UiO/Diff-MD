...
def add_mol_section(section, params, name):
    if params:
        converted_list = [
            [value for key, value in elem.__dict__.items() if not key.startswith("_")]
            for elem in params
        ]
...
def add_raw_list_section(section, entries, name):
    """Like ``add_mol_section`` but for plain ``list[list]`` data
    (CMAP), where entries don't have a ``__dict__`` to introspect.
    """
    if entries:
        section.add(name, toml_add_lines(list(entries)))
        section.add(tomlkit.nl())
...
def write_itp_toml(molname, params, dirname):
    ...
    # already a plain list, so serialize directly.
    add_raw_list_section(molecule, getattr(params, "cmap", None), "cmap")
